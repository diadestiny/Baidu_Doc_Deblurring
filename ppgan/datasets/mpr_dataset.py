...
class MPRTrain(Dataset):
    def __init__(self, rgb_dir, img_options=None):
        ...
        inp_files = sorted(os.listdir(os.path.join(rgb_dir, 'blur_image')))
        tar_files = sorted(os.listdir(os.path.join(rgb_dir, 'gt_image')))

        self.inp_filenames = [
            os.path.join(rgb_dir, 'blur_image', x) for x in inp_files
            if is_image_file(x)
        ]
        self.tar_filenames = [
            os.path.join(rgb_dir, 'gt_image', x) for x in tar_files
            if is_image_file(x)
        ]
